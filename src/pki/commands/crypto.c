...
int gmsdf_crypter_test(chunk_t data)
{
	...
	if (!crypter->decrypt(crypter, d, iv, &response))
	{
	...
	if (!crypter->encrypt(crypter, response, iv, &d))
	{
...
int gmsdf_ran_test(chunk_t data)
{
	int ret = 0;
	rng_t *rng;
	chunk_t response = chunk_empty;
...
static void __attribute__ ((constructor))reg()
{
	command_register((command_t)
		...
		{"[--in file] "
		 "[--type hash|crypter|prf|signer|dh|ran]"},
		{
